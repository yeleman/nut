...
def report_validation(request, report_receipt):  # entity, slug, year, month):
    ...
    formslist = []
    if report.is_samp:
        formslist.append(('pec_samp_form',
                          PECSAMPReportForm(data,
                                            instance=reports['pec_samp_report'],
                                            prefix='pec_samp_report')))
    if report.is_sam:
        formslist.append(('pec_sam_form',
                          PECSAMReportForm(data,
                                           instance=reports['pec_sam_report'],
                                           prefix='pec_sam_report')))
    if report.is_mam:
        formslist.append(('pec_mam_form',
                          PECMAMReportReportForm(data,
                                                 instance=reports['pec_mam_report'],
                                                 prefix='pec_mam_report')))
    formslist.append(('pec_other_form',
                      PECOthersReportForm(data,
                                          instance=reports['pec_other_report'],
                                          prefix="pec_other_report")))
    if report.is_samp:
        formslist.append(('cons_samp_form',
                          InputConsumptionReportFormSet(data,
                                                        prefix='cons_samp_report',
                                                        instance=reports['cons_samp_report'])))
    if report.is_sam:
        formslist.append(('cons_sam_form',
                          InputConsumptionReportFormSet(data,
                                                        prefix='cons_sam_report',
                                                        instance=reports['cons_sam_report'])))
    if report.is_mam:
        formslist.append(('cons_mam_form',
                          InputConsumptionReportFormSet(data,
                                                        prefix='cons_mam_report',
                                                        instance=reports['cons_mam_report'])))
    if report.is_samp:
        formslist.append(('order_samp_form',
                          InputOrderReportFormSet(data,
                                                  prefix='order_samp_report',
                                                  instance=reports['order_samp_report'])))
    if report.is_sam:
        formslist.append(('order_sam_form',
                          InputOrderReportFormSet(data,
                                                  prefix='order_sam_report',
                                                  instance=reports['order_sam_report'])))
    if report.is_mam:
        formslist.append(('order_mam_form',
                          InputOrderReportFormSet(data,
                                                  prefix='order_mam_report',
                                                  instance=reports['order_mam_report'])))
    forms = SortedDict(tuple(formslist))
    ...
    for form in forms_with_instances:
        valid = form.is_valid()
        print(u"%s: %s" % (form.__class__, valid))
        if not valid:
            print(form.errors)
            try:
