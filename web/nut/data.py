...
def get_reports_to_validate(entity, period=current_reporting_period()):
    ...
    return [(report.entity, report) \
            for report \
            in NutritionReport.unvalidated\
...
def get_not_received_reports(entity, period=current_reporting_period()):
    ...
    reports = NutritionReport.objects.filter(entity__in=entity.get_children(), \
                                           period=period)
...
def most_accurate_report(provider, period=current_reporting_period()):
    ...
    try:
        return NutritionReport.validated.filter(period=period)[0]
    except:
        return None
...
def raw_data_periods_for(entity):
    ...
    return [r.mperiod for r in NutritionReport.validated.filter(entity=entity)]
