...
def export_report_stream(report):
    ...
    if is_django:
        ...
        ior_class = list(report.order_reports()[0]\
                    .input_order_reports.all())[0].__class__.objects.filter
    else:
    ...
    def create_values(hc, liste, prefix):
        liste_val = [val(hc, "%s%s" % (prefix, v)) for v in liste]
        return liste_val
    ...
    l_val = ['_total_beginning', '_total_beginning_m', '_total_beginning_f',
             ...
             '_muac_u11_muac_u18', '_oedema', '_other', '_new_case',
             '_relapse', '_returned', '_nut_transfered_in', '_nut_referred_in',
             '_admitted', '_admitted_m', '_admitted_f']
    # Valeurs tableau1
    # URENI 1
    write_p(empty(16), 9, 0, 1, 1)
    # < 6 mois
    write_p(create_values(report.pec_samp_report, l_val, 'u6'),
                                                     10, 0, 1, 1, style_value)
    # 6-59 mois
    write_p(create_values(report.pec_samp_report, l_val, 'u59'),
                                                     11, 0, 1, 1, style_value)
    # > 59 mois
    write_p(create_values(report.pec_samp_report, l_val, 'o59'),
                                                     12, 0, 1, 1, style_value)

    # URENAS 2
    write_p(empty(16), 13, 0, 1, 1)
    # 6-59 mois
    write_p(create_values(report.pec_sam_report, l_val, 'u59'),
                                                     14, 0, 1, 1, style_value)
    # > 59 mois
    write_p(create_values(report.pec_sam_report, l_val, 'o59'),
                                                     15, 0, 1, 1, style_value)
    # SUIVI DE URENI 1
    write_p(create_values(report.pec_sam_report, l_val, 'fu1'),
                                                     16, 0, 1, 1, style_value)

    # URENAM 3
    write_p(empty(16), 17, 0, 1, 1)
    # 6-59 mois
    write_p(create_values(report.pec_mam_report, l_val, 'u59'),
                                                     18, 0, 1, 1, style_value)
    # > FE/FA
    write_p(create_values(report.pec_mam_report, l_val, 'pw'),
                                                     19, 0, 1, 1, style_value)
    # SUIVI DE 1 ET 2
    write_p(create_values(report.pec_mam_report, l_val, 'fu12'),
                                                     20, 0, 1, 1, style_value)
    # TOTAL
    write_p(create_values(report, l_val, 'sum_all'), 21, 0, 1, 1, style_value)
    ...
    l_val1 = ['_healed', '_referred_out', '_deceased', '_aborted',
              '_non_respondant', '_medic_transfered_out',
              '_nut_transfered_out', '_total_out', '_total_out_m',
              '_total_out_f', '_total_end', '_total_end_m', '_total_end_f']
    # URENI 1
    write_p(empty(12), 27, 0, 1, 1)
    # < 6 mois
    write_p(create_values(report.pec_samp_report, l_val1, 'u6'),
                                                     28, 0, 1, 1, style_value)
    # < 6-59  mois
    write_p(create_values(report.pec_samp_report, l_val1, 'u59'),
                                                     29, 0, 1, 1, style_value)
    # > 59  mois
    write_p(create_values(report.pec_samp_report, l_val1, 'o59'),
                                                     30, 0, 1, 1, style_value)

    # URENAS 2
    write_p(empty(12), 31, 0, 1, 1)
    # < 6-59  mois
    write_p(create_values(report.pec_sam_report, l_val1, 'u59'),
                                                     32, 0, 1, 1, style_value)
    # > 59 mois
    write_p(create_values(report.pec_sam_report, l_val1, 'o59'),
                                                     33, 0, 1, 1, style_value)
    # SUIVI DE URENI 1
    write_p(create_values(report.pec_sam_report, l_val1, 'fu1'),
                                                     34, 0, 1, 1, style_value)

    # URENAM 3
    write_p(empty(12), 35, 0, 1, 1)
    # 6-59 mois
    write_p(create_values(report.pec_mam_report, l_val1, 'u59'),
                                                     36, 0, 1, 1, style_value)
    # > FE/FA
    write_p(create_values(report.pec_mam_report, l_val1, 'pw'),
                                                     37, 0, 1, 1, style_value)
    # SUIVI DE 3 ET 2
    write_p(create_values(report.pec_mam_report, l_val1, 'fu12'),
                                                     38, 0, 1, 1, style_value)
    # TOTAL
    ...
    liste_title = [u"REGION", u"District", u"CSCom",
                   u"TYPE DE CENTRE", u"Mois"]
    ...
    sheet.write(7, 0, u"Les champs ci-dessus sont à remplir dans "
                      u"la feuille PEC.")
    ...
    sheet.write(28, 0, u"Solde fin du mois = [Stock debut du mois + Stock "
                       u"recu (facultatif)] - [Stock utilisé + Stock perdu/"
                       u"avarié (facultatif)]", styleheader_left_g)
